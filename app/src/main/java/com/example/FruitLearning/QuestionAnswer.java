package com.example.FruitLearning;

public class QuestionAnswer {
    public static String question[] ={
            "Quả gì tên gọi như hoa\n" +
            "Thu về chín đỏ như sơn son?",
            "Có mùi, có khía vàng au\n" +
            "Chim khôn ăn quả hẹn sau trả vàng.\n" +
            "Là quả gì?",
            "Quả gì nhiều mắt\n" +
            "Khi chín nứt ra\n" +
            "Ruột trắng nõn nà\n" +
            "Hạt đen nhanh nhánh?",
            "Khi chim tu hú gọi bầy\n" +
            "Là mùa quả chín đỏ đầy quê ta.\n" +
            "Là quả gì?",
            "Quả gì thường ở trên giàn\n" +
            "Từng chùm chín mọng mang toàn chữ o?",
            "Quê em ở chốn đảo xa\n" +
            "An Tiêm thuở ấy làm quà tặng vua.\n" +
            "Là quả gì?",
            "Da xanh ngăn ngắt\n" +
            "Nổi tiếng chua ngoa\n" +
            "Nhiều người vẫn quý bảo là thơm ngon?",
            "Hoa mỏng manh sắc tím tươi\n" +
            "Trái chín gần mọng ven đồi gần xa.",
            "Da cóc mà bọc trứng gà\n" +
            "Bổ ra thơm phức, cả nhà muốn ăn\n" +
            "Là quả gì?",
            "Quả gì ở tận trên cao\n" +
            "Không phải giếng đào mà có nước trong?"
    };

    public static String choices[][] = {
            {"Quả gấc","Quả mận","Quả táo","Quả hồng"},
            {"Quả khế","Quả lê","Quả đào","Quả chuối"},
            {"Quả na","Quả vải","Quả nhãn","Quả xoài"},
            {"Quả na","Quả vải","Quả lựu","Quả ổi"},
            {"Quả vải","Quả ổi", "Quả nho", "Quả dừa"},
            {"Củ khoai lang", "Quả khế", "Quả dưa hấu", "Quả dưa gang"},
            {"Quả ổi", "Quả bưởi", "Quả chanh", "Quả bí đao"},
            {"Quả cà", "Quả sim", "Quả mướp", "Quả thông"},
            {"Quả vãi", "Quả xoài", "Quả bưởi", "Quả mít"},
            {"Quả bưởi", "Quả dừa", "Quả lê", "Quả vải"}
    };

    public static String correctAnswers[] = {
            "Quả hồng",
            "Quả khế",
            "Quả na",
            "Quả vải",
            "Quả nho",
            "Quả dưa hấu",
            "Quả chanh",
            "Quả sim",
            "Quả mít",
            "Quả dừa"
    };

}
